defmodule Cldr.PersonName.FormatTest do
  ...
  for test <- Cldr.PersonName.TestData.parse("de"), test.line in @tests do
    with {:ok, test_locale} <- AllBackend.Cldr.validate_locale(test.locale),
      ...
      test "##{test.line} with params #{inspect test.params} formats to #{inspect test.expected_result}" do
